...
export async function create(contactData: ContactCreateInput): Promise<Contact> {
...
export async function findAll(limit: number = 100, offset: number = 0): Promise<Contact[]> {
...
export async function findByFormType(formType: 'contact' | 'brochure', limit: number = 100): Promise<Contact[]> {
...
export async function updateMessage(id: string, message: string): Promise<Contact> {
    const query = `
      UPDATE contacts 
